...
# This script assumes that the UCD is controlling VCS rails as GPIOs 12 and 1.
...
{
    unbind_ucd
    echo -e "\tSetting UCD GPIO 12 to $1"
    ucd_set 0xFA 12
    ucd_set 0xFB $1
    ucd_set 0xFB $1
    echo -e "\tSetting UCD GPIO 1 to $1"
    ucd_set 0xFA 1
    ucd_set 0xFB $1
    ucd_set 0xFB $1
    rebind_ucd
...
vcs_get()
{
    ...
    echo -e "\tUCD GPIO 12 state=$val"
    ucd_set 0xFA 1
    ucd_get 0xFB
    local val=`echo $ucd_reg | grep -i -c 0x0f`
    echo -e "\tUCD GPIO 1 state=$val"
    rebind_ucd
}
