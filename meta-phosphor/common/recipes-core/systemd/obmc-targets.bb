...
GPIO_TGTFMT = "obmc-gpio-asserted@0.target"
...
SYSTEMD_LINK_${PN} += "obmc-gpio-asserted@.target:obmc-gpio-asserted@${OBMC_CHECKSTOP_INSTANCES}.target"
#SYSTEMD_LINK_${PN} += "${@compose_list(d, 'GPIO_QUIESCE_FMT', OBMC_CHECKSTOP_INSTANCES)}"
SYSTEMD_LINK_${PN} += "${@compose_list(d, 'GPIO_QUIESCE_FMT')}"
