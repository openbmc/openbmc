...
# - termination: Target to run when host terminates. it is very much similar to
#                quiesce target but the only delta is that this target contains
#                multiple services and one of them is the quiesce target.
HOST_ACTION_TARGETS = "start stop quiesce reset shutdown termination"
...
TERM_TGTFMT = "obmc-host-termination@{0}.target"
QUIESCE_INSTFMT = "obmc-host-quiesce@{0}.target"
TERM_QUIESCE_FMT = "${QUIESCE_TMPL}:${TERM_TGTFMT}.wants/${QUIESCE_INSTFMT}"
...
SYSTEMD_LINK_${PN} += "${@compose_list(d, 'TERM_QUIESCE_FMT', 'OBMC_CHECKSTOP_INSTANCES')}"
