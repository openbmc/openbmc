...
# Force the host reboot target to run the chassis poweroff target
HOST_REBOOT_TGTFMT = "obmc-host-reboot@{1}.target"
HOST_REBOOT_FMT = "../${CHASSIS_STOP_TMPL}:${HOST_REBOOT_TGTFMT}.requires/${CHASSIS_STOP_INSTFMT}"
SYSTEMD_LINK_${PN}-host += "${@compose_list_zip(d, 'HOST_REBOOT_FMT', 'OBMC_CHASSIS_INSTANCES', 'OBMC_HOST_INSTANCES')}"
