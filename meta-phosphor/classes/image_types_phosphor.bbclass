...
python() {
    ...
    leb_size = int(peb_size) -2
    d.setVar('FLASH_LEB_SIZE', str(leb_size))

    rwfs_size = d.getVar('FLASH_UBI_RWFS_SIZE', True)
    rwfs_size = int(rwfs_size) * 1024
    lebs = int(rwfs_size / (leb_size * 1024))
...
UBIFS_RWFS_CMD = "mkfs.ubifs -r ubifs -c ${FLASH_UBI_RWFS_LEBS} -m ${FLASH_PAGE_SIZE} -e ${FLASH_LEB_SIZE}KiB rwfs.ubifs"
...
do_generate_ubi() {
	...
	add_volume $cfg 2 dynamic rwfs rwfs.${FLASH_UBI_OVERLAY_BASETYPE}
	...
	truncate -s ${FLASH_SIZE}K ${IMGDEPLOYDIR}/${IMAGE_NAME}.ubi.mtd
